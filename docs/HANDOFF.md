...
**追記 2026-08-17 ✅実測: キャッシュを壊すのは「UI からの停止・一時停止」である。**
当初これを「約 43 時間のアイドルで失効した」と書いたが**誤りだった**ので訂正する。切り分けの結果:

| 何が起きたか | キャッシュ |
| --- | --- |
| lerobot が `stopScript()` → Loop Program が再起動 | **保たれる** ✅（実測: ロールアウト直後も `2:PLAYING` のまま、次のコマンドも即接続） |
| **UI の停止/一時停止ボタン** | **破棄される** ✅（ノードが黄色に戻る） |

ログ上は UI 操作が `PATCH /…/program/execution/0/0/**DRAFT**` として現れる。
プログラムが DRAFT 状態に戻ると、差し込まれていた URScript が落ちる、という筋。
43 時間の件も、実際にはその間に UI から停止されていたことが後で分かった
（`PROGRAM_XXX_STOPPED` がブラウザのアクセスと同時刻に記録されていた）。

**実務上の含意: URSim の画面は「見る」だけにして、停止/一時停止ボタンを押さない。**
押すと Update program からやり直しになる。

なお `runtime_state` は 3 つを区別できるので、まずこれを見ること:
`2:PLAYING` = 正常 / `1:STOPPED`・`4:PAUSED` = ▶ が要る（かつノードが黄色ならキャッシュ再取得も要る）。
ただし **PLAYING はノードが有効であることまでは保証しない**（DRAFT に戻る前の残骸で PLAYING を返す場合がある）ので、
繋がらないときは UI でノードの色も見る。
